...
/**
 * This static class manages table statistics generated by creation of ExtVP
 * model
 * 
...
public class SparkTableStatistics {
	...
	public static HashMap<String, Integer> predicates = new HashMap<String, Integer>();
	...
	private static void readPredicates() {
		try {
			BufferedReader reader = new BufferedReader(new FileReader(Tags.PREDICATE_MAP));
			String line;

			while ((line = reader.readLine()) != null) {
				String[] temp = line.split(" -> ");
				predicates.put(temp[0], Integer.parseInt(temp[1]));
			}
			reader.close();
		} catch (Exception e) {
			System.err.format("Exception occurred trying to read '%s'.", Tags.PREDICATE_MAP);
			e.printStackTrace();
		}
	...
	/**
	 * Compose tableName VP <pred1> ExtVP <pred1><pred2>
	 * 
	 * @param candidate
	 *            String{[relType], [pred1], [pred2]}
	 * @return table name
	 */
	private static String composeTableName(String[] candidate) {
		String ret = "";
		if (candidate[0].equals("VP")) {
			ret = ("<" + candidate[1] + ">").replace("<<", "<").replace(">>", ">");
		} else {
			ret = ("<" + candidate[1] + "><" + candidate[2] + ">").replace("<<", "<").replace(">>", ">");
		}
		return ret;
	}
	 ...
	 *            for certain table in SQL query
	 ...
	 * 
	 * @param elmTP1
	 *            = linked triple element(sub/obj) of the main(first) TP
	 * @param elmTP2
	 *            = linked triple element(sub/obj) sub/obj of the slave(second) TP
	...
	/**
	 * Get original table name in the form how it appeared in SQL query before it
	 * was replaced by corresponding place holder
	 * 
	 * By the way, place holder format: <original_tablename>$$<place_holder_ID>$$
	 * 
	 * @param tableName
	 *            (can be place holder(if already replaced) or original name)
	...
	/**
	 * !!!Debugging Output!!! Prints all candidate tables for all place holder in
	 * SQL query
	 */
	private static void listAllCandidateTables() {
		for (String plcHolder : tableCandidatesMap.keySet()) {
			...
			for (int i = 0; i < candList.getSqlTableCandidates().size(); i++) {
				...
				System.out.println("Name: " + candList.placeHolderName + " Type: " + candidate[0] + " " + candidate[1]
						+ " " + candidate[2] + " ");
			}
		}
	}
	 ...
	 * generates a list of table candidates(ExtVP/VP tables) for every original SQL
	 * table, which can be found in from clause of corresponding subquery.
	 * 
	 * @param list:
	 *            A list of SQL subqueries objects, which have to be joined
	 */
	public static void generateCandidatesTableLists(ArrayList<SqlStatement> joinList) {
		...
		for (int i = 0; i < joinList.size(); i++) {
			...
			for (int j = 0; j < joinList.size(); j++)
				...
				if (i != j && joinList.get(i).getType().equals("Select")
						&& joinList.get(j).getType().equals("Select")) {
					...
					for (String alias1 : tr1.keySet()) {
						for (String alias2 : tr2.keySet()) {
							...
							if (tr1.get(alias1).equals(tr2.get(alias2)) && !relationType.equals("UNDEFINED")) {
								candList.addExtVPCandidateTable(relationType,
										getOriginalTableName(joinList.get(i).getFrom()),
										getOriginalTableName(joinList.get(j).getFrom()));
							}
						}
					}
	...
	/**
	 * Read VP tables statistics FileFormat: ... ------------------------... //
	 * Start table statistics <tableName>\t<VP table size>\t<TT table
	 * size>\t<scale(VP/TT)> ... ------------------------... // End table statistics
	 * ...
	 * 
	...
	private static long readVPStatistic() {
		...
		try {
			...
			while ((line = reader.readLine()) != null && !line.contains("-------------")) {
				...
				String tempname=temp[0].replaceFirst("<", "");
				tempname=tempname.substring(0, tempname.length()-1);
				vpStats.put(("<prop"+predicates.get(tempname)+">"), newStat);
			}
			reader.close();
		} catch (Exception e) {
		...
		// System.out.println(vpStats.get("<mo:conductor>").size+"/"+vpStats.get("<mo:conductor>").sizeSource+"
		// "+vpStats.get("<mo:conductor>").freq);
	...
	private static String getExtVPStatFileDest(String relType) {
		if (relType.equals("SO"))
			return Tags.SO_TABLE_STAT;
		if (relType.equals("OS"))
			return Tags.OS_TABLE_STAT;
		if (relType.equals("SS"))
			return Tags.SS_TABLE_STAT;
		return null;
	}

	/**
	 * Read ExtVP tables statistics FileFormat: ... ------------------------... //
	 * Start table statistics <tableName>\t<ExtVP table size>\t<VP table
	 * size>\t<scale(ExtVP/VP)>\t<scale(VP/TT)> ... ------------------------... //
	...
	private static long readExtVPStatistic(String relType) {
		...
		try {
			...
			while ((line = reader.readLine()) != null && !line.contains("-------------")) {
				...
				String tempname=temp[0].replaceFirst("<", "");
				tempname=tempname.substring(0, tempname.length()-1);
				String[] pair=tempname.split("><");
				if (relType.equals("SO"))
					soStats.put(("<prop"+predicates.get(pair[0])+">"+"<prop"+predicates.get(pair[1])+">"), newStat);
				else if (relType.equals("OS"))
					osStats.put(("<prop"+predicates.get(pair[0])+">"+"<prop"+predicates.get(pair[1])+">"), newStat);
				else if (relType.equals("SS"))
					ssStats.put(("<prop"+predicates.get(pair[0])+">"+"<prop"+predicates.get(pair[1])+">"), newStat);
				System.out.println(tempname+soStats);
			}
		...
		if (relType.equals("SO"))
			System.out.println("SO STAT Size = " + soStats.size());
		else if (relType.equals("OS"))
			System.out.println("OS STAT Size = " + osStats.size());
		else if (relType.equals("SS"))
			System.out.println("SS STAT Size = " + ssStats.size());

		return allTriplesNumber;
	}

	/**
	 * Determines id of the best table for every list of candidate tables. Best
	 * table is a table having smallest number of triples Triple numbers of tables
	 * can be obtained from read table statistics
	 */
			...
			for (int i = 0; i < candList.getSqlTableCandidates().size(); i++) {
				...
				if (candidate[0].equals("VP")) { // VP candidate table
					...
					if (candidate[0].equals("SO")) {
						if (!soStats.containsKey(tName))
							continue;
						for (String huj : soStats.keySet())
							System.out.println("XUJ->" + huj);
						...
						if (extVpTableSize < min && extVpTableSize < vpTableSize * Tags.ScaleUB && Tags.ALLOW_SO) {
							min = extVpTableSize;
							candList.bestCandidateID = i;
						}
					} else if (candidate[0].equals("OS")) {
						if (!osStats.containsKey(tName))
							continue;

						extVpTableSize = osStats.get(tName).size;
						vpTableSize = osStats.get(tName).sizeSource;

						if (extVpTableSize < min && extVpTableSize < vpTableSize * Tags.ScaleUB && Tags.ALLOW_OS) {
							min = extVpTableSize;
							candList.bestCandidateID = i;
						}
					} else if (candidate[0].equals("SS")) {
						if (!ssStats.containsKey(tName))
							continue;

						extVpTableSize = ssStats.get(tName).size;
						vpTableSize = ssStats.get(tName).sizeSource;

						if (extVpTableSize < min && extVpTableSize < vpTableSize * Tags.ScaleUB && Tags.ALLOW_SS) {
							min = extVpTableSize;
							candList.bestCandidateID = i;
						}
					}
	...
	/**
	 * Check if the ExtVP table corresponds to an empty relation Table statistics
	 * contains information about all possible ExtVP tables (even about tables,
	 * which aren't saved) -> if tableName does not occur in any statistic then it
	 * corresponds to an empty relation
	 * 
	...
	private static Boolean emptyRelationCheck(String relType, String extVpTableName) {
		...
		if (relType.equals("SO")) {
			if (!soStats.containsKey(extVpTableName))
				return true;
		} else if (relType.equals("OS")) {
			if (!osStats.containsKey(extVpTableName))
				return true;
		} else if (relType.equals("SS")) {
			if (!ssStats.containsKey(extVpTableName))
				return true;
		}
	 ...
	 * These instructions contain table candidates lists and corresponding best
	 * tables for any place holder in SQL query.
	 * 
	 * If the query contains empty relation (relation, which doesn't exist in the
	 * input RDF set), than function returns empty instructions, what indicates an
	 * empty result list
	 * 
	...
	public static String generateTablesUsageInstructions(String query) {
		...
		for (String placeHolderName : tableCandidatesMap.keySet()) {
			...
			// String newPlaceholder = placeHolderName;
			// query = query.replace(placeHolderName, newPlaceholder);
			res += placeHolderName + "\t" + candList.bestCandidateID + "\t" + bestCandidate[0] + "\t" + bestCandidate[1]
					+ "/" + bestCandidate[2] + "\n";
